...
%&RSUFile.IncludeSASCodeIn(i_dir_path = &G_APP_ROOT_S5./&G_APP_S5_PROGRAM_ROOT_DIR./macros
									...
									, i_is_silent_mode = %&RSUBool.False
									, i_priority_files = Constant.sas EnvironmentManager.sas)
...
data WORK.formula_execution_controller;
	...
	;
datalines;
0 HzImpact 0F Time{ホライズン}>20190331
0 BSPL 09 1
0 Rating 12 Time{ホライズン}>=20190331
0 PD 11 Time{ホライズン}>20190331
1 Collateral 0A Time{ホライズン}>20190331
0 EL 0D Time{ホライズン}>20190331
0 DeltaEL 0C Time{ホライズン}>20190331
;
run;
quit;
%macro Workflow__testrun;
	%local /readonly _UI_PROCESS_NAME = PhysicalRisk;
